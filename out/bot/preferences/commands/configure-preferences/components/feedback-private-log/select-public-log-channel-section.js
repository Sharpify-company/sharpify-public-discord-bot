...
Object.defineProperty(exports, "SelectFeedbackPrivateLogChannel", {
    enumerable: true,
    get: function() {
        return SelectFeedbackPrivateLogChannel;
    }
});
...
let SelectFeedbackPrivateLogChannel = class SelectFeedbackPrivateLogChannel {
    ...
    async createSelectChannel() {
        const store = await (0, _sharpify.getLocalStoreConfig)();
        const preferences = store.getPreferences();
        const selectMenu = new _discord.ChannelSelectMenuBuilder().setCustomId(`select_feedback_private_log_channel`).setPlaceholder("Selecione qual canal o log privado vai ser mandado...").setMaxValues(1).setMinValues(1);
        const defaultPlaceholder = "Selecione qual canal o log privado vai ser mandado...";
        if (preferences.feedbackPrivateLog.channelId) {
            const channel = await this.client.channels.fetch(preferences.feedbackPrivateLog.channelId).catch(()=>null);
            if (channel) {
...
_ts_decorate([
    (0, _necord.ChannelSelect)("select_feedback_private_log_channel"),
...
], SelectFeedbackPrivateLogChannel.prototype, "handleChannelSelected", null);
SelectFeedbackPrivateLogChannel = _ts_decorate([
