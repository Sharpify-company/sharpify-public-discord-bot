...
class DiscordUserRepository {
	async create(discordUserEntity: DiscordUserEntity): Promise<void> {
      ...
      `);
		insert.run(discordUserEntity.id, discordUserEntity.cartChannelId, discordUserEntity.cartMessageId);
	}
	...
	async update(discordUserEntity: DiscordUserEntity): Promise<void> {
      ...
      WHERE id = ?
    `);
		update.run(discordUserEntity.cartChannelId, discordUserEntity.cartMessageId, discordUserEntity.id);
	}
}
