...
Object.defineProperty(exports, "SelectConfirmDeliveryLogChannel", {
    enumerable: true,
    get: function() {
        return SelectConfirmDeliveryLogChannel;
    }
});
...
let SelectConfirmDeliveryLogChannel = class SelectConfirmDeliveryLogChannel {
    async handleChannelSelected([interaction], selected) {
        ...
        preferences.confirmDelivery.channelId = channel.id;
    ...
    async createSelectChannel() {
        const store = await (0, _sharpify.getLocalStoreConfig)();
        const preferences = store.getPreferences();
        const selectMenu = new _discord.ChannelSelectMenuBuilder().setCustomId(`select_confirm_delivery_log_channel`).setPlaceholder("Selecione qual canal o log de confirmação de entrega vai ser mandado...").setMaxValues(1).setMinValues(1);
        const defaultPlaceholder = "Selecione qual canal o log de confirmaçõa de entrega vai ser mandado...";
        if (preferences.confirmDelivery.channelId) {
            const channel = await this.client.channels.fetch(preferences.confirmDelivery.channelId).catch(()=>null);
            if (channel) {
...
_ts_decorate([
    (0, _necord.ChannelSelect)("select_confirm_delivery_log_channel"),
...
], SelectConfirmDeliveryLogChannel.prototype, "handleChannelSelected", null);
SelectConfirmDeliveryLogChannel = _ts_decorate([
...
//# sourceMappingURL=select-confirm-delivery-log-channel-section.js.map