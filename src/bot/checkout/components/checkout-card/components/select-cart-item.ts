...
export class SelectCartItemComponent {
	...
	async createSelect({ discordUserId, defaultItemId }: { discordUserId: string; defaultItemId?: string }) {
		...
		const options = checkoutCartItems.map((item) => ({
			label: formatCheckoutCartItemNameHelper(item).slice(0, 100), // Discord limit: 100 characters
			description: `💸 Valor: ${formatPrice(item.item.pricing.price)} | 📦 Estoque ${item.item.inventory.stockQuantity === null ? "Ilimitado" : `${item.item.inventory.stockQuantity} unidades`}`.slice(0, 100), // Discord limit: 100 characters
