...
Object.defineProperty(exports, "SelectSalesPrivateLogChannel", {
    enumerable: true,
    get: function() {
        return SelectSalesPrivateLogChannel;
    }
});
...
let SelectSalesPrivateLogChannel = class SelectSalesPrivateLogChannel {
    async handleChannelSelected([interaction], selected) {
        ...
        await interaction.update(await this.buildPreferenceConfigure.build({
            section: "SALES_PRIVATE_LOG"
        }));
    }
    async createSelectChannel() {
        const store = await (0, _sharpify.getLocalStoreConfig)();
        const preferences = store.getPreferences();
        const selectMenu = new _discord.ChannelSelectMenuBuilder().setCustomId(`select_sales_private_log_channel`).setPlaceholder("Selecione qual canal o log privado vai ser mandado...").setMaxValues(1).setMinValues(1);
        const defaultPlaceholder = "Selecione qual canal o log privado vai ser mandado...";
        if (preferences.privateLogSales.channelId) {
            const channel = await this.client.channels.fetch(preferences.privateLogSales.channelId).catch(()=>null);
            if (channel) {
...
_ts_decorate([
    (0, _necord.ChannelSelect)("select_sales_private_log_channel"),
...
], SelectSalesPrivateLogChannel.prototype, "handleChannelSelected", null);
SelectSalesPrivateLogChannel = _ts_decorate([
...
//# sourceMappingURL=select-private-log-channel-section.js.map