...
declare class RequestHelper {
	private options;
	constructor(options: { baseURL: string; storeId: string; apiToken: string });
...
declare class Product {
	...
	>;
	listSimilarProducts(input: { productId: string }): Promise<
		ActionsOutput<{
			products: ProductProps[];
		}>
	>;
	get(input: { id: string; slug?: string; idOrSlug?: string; includeNonListed?: boolean }): Promise<
		ActionsOutput<{
			product: ProductProps;
		}>
	>;
	decreseStock(input: { productId: string; productItemId: string; quantity: number }): Promise<
		ActionsOutput<{
			stock: string;
			type: ProductProps.StockType;
		}>
	>;
	listMyResellersProducts(): Promise<
		ActionsOutput<{
			products: ProductProps[];
		}>
	>;
...
declare class ExternalEvents {
	private options;
	constructor(options: SharpifyOptions);
	listPendingEvents(): Promise<
		ActionsOutput<{
			events: ExternalEventsProps[];
		}>
	>;
	markAsReceived(input: { ids: string[] }): Promise<ActionsOutput>;
}

declare class Roblox {
	private options;
	constructor(options: SharpifyOptions);
	getUserByUsername(input: { username: string }): Promise<
		ActionsOutput<{
			id: number;
			name: string;
			displayName: string;
...
declare class Coupon {
	private options;
	constructor(options: SharpifyOptions);
	validateCoupon(input: { code: string }): Promise<
		ActionsOutput<{
			coupon: CouponProps;
		}>
	>;
...
declare class Affiliate {
	...
	createMyAffiliateCode(input: { code: string }): Promise<ActionsOutput>;
	updateMyAffiliateCode(input: { code: string; affiliateCodeId: string }): Promise<ActionsOutput>;
	deleteMyAffiliateCode(input: { affiliateCodeId: string }): Promise<ActionsOutput>;
	listMyWithdraws(input: { page: number; limit: number }): Promise<
		ActionsOutput<{
			withdraws: AffiliateWithdrawProps[];
			lastPage: number;
		}>
	>;
	requestMyWithdraw(input: { amount: number; withdrawInfo: AffiliateWithdrawProps.WithdrawInfo }): Promise<ActionsOutput>;
	cancelMyWithdraw(input: { withdrawId: string }): Promise<ActionsOutput>;
}
...
declare class Auth {
	...
	oauthChangeCodeForToken(input: { code: string; platform: "GOOGLE" | "DISCORD" }): Promise<
		ActionsOutput<{
			accessToken: string;
		}>
	>;
	defaultSendVerificationCodeToEmail(input: { email: string }): Promise<ActionsOutput>;
	defaultVerifyCode(input: { code: string; email: string }): Promise<
		ActionsOutput<{
			accessToken: string;
		}>
	>;
...
declare class Order {
	...
	getOrder(input: { orderId: string }): Promise<
		ActionsOutput<{
			order: OrderProps;
		}>
	>;
	customerListMyOrders(input: { page: number; limit: number }): Promise<
		ActionsOutput<{
			orders: OrderProps[];
			lastPage: number;
		}>
...
declare class Feedback {
	private options;
	constructor(options: SharpifyOptions);
	listProductFeedbacks(input: { page: number; limit: number; productId: string; star?: number | string }): Promise<
		ActionsOutput<{
...
declare class Analytics {
	private options;
	constructor(options: SharpifyOptions);
	createSession(input: { sessionId: string }): Promise<
		ActionsOutput<{
			id: string;
		}>
	>;
