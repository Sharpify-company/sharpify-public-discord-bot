...
Object.defineProperty(exports, "SelectFailLogChannel", {
    enumerable: true,
    get: function() {
        return SelectFailLogChannel;
    }
});
...
let SelectFailLogChannel = class SelectFailLogChannel {
    async handleChannelSelected([interaction], selected) {
        ...
        preferences.failLog.channelId = channel.id;
    ...
    async createSelectChannel() {
        const store = await (0, _sharpify.getLocalStoreConfig)();
        const preferences = store.getPreferences();
        const selectMenu = new _discord.ChannelSelectMenuBuilder().setCustomId(`select_fail_log_channel`).setPlaceholder("Selecione qual canal o log de falhas vai ser mandado...").setMaxValues(1).setMinValues(1);
        const defaultPlaceholder = "Selecione qual canal o log de falhas vai ser mandado...";
        if (preferences.failLog.channelId) {
            const channel = await this.client.channels.fetch(preferences.failLog.channelId).catch(()=>null);
            if (channel) {
...
_ts_decorate([
    (0, _necord.ChannelSelect)("select_fail_log_channel"),
...
], SelectFailLogChannel.prototype, "handleChannelSelected", null);
SelectFailLogChannel = _ts_decorate([
...
], SelectFailLogChannel);

//# sourceMappingURL=select-fail-log-channel-section.js.map