...
Object.defineProperty(exports, "SelectFeedbackPublicLogChannel", {
    enumerable: true,
    get: function() {
        return SelectFeedbackPublicLogChannel;
    }
});
...
let SelectFeedbackPublicLogChannel = class SelectFeedbackPublicLogChannel {
    async handleChannelSelected([interaction], selected) {
        ...
        await interaction.update(await this.buildPreferenceConfigure.build({
            section: "FEEDBACK_PUBLIC_LOG"
        }));
    }
    async createSelectChannel() {
        const store = await (0, _sharpify.getLocalStoreConfig)();
        const preferences = store.getPreferences();
        const selectMenu = new _discord.ChannelSelectMenuBuilder().setCustomId(`select_feedback_public_log_channel`).setPlaceholder("Selecione qual canal o log publico vai ser mandado...").setMaxValues(1).setMinValues(1);
        const defaultPlaceholder = "Selecione qual canal o log publico vai ser mandado...";
        if (preferences.feedbackPublicLog.channelId) {
            const channel = await this.client.channels.fetch(preferences.feedbackPublicLog.channelId).catch(()=>null);
            if (channel) {
...
_ts_decorate([
    (0, _necord.ChannelSelect)("select_feedback_public_log_channel"),
...
], SelectFeedbackPublicLogChannel.prototype, "handleChannelSelected", null);
SelectFeedbackPublicLogChannel = _ts_decorate([
