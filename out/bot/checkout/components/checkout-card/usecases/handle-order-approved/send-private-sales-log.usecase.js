...
Object.defineProperty(exports, "SendPrivateSalesLogUsecase", {
    enumerable: true,
    get: function() {
        return SendPrivateSalesLogUsecase;
    }
});
...
let SendPrivateSalesLogUsecase = class SendPrivateSalesLogUsecase {
    async execute({ discordUserId, orderProps }) {
        ...
        const channel = await guild.channels.fetch(storePreferences.privateLogSales.channelId).catch(()=>null);
        if (!channel || channel.type !== _discord.ChannelType.GuildText) return;
        const embed = new _discord.EmbedBuilder().setColor(_config.BotConfig.color).setTitle("🛒 Nova compra feita. (Log privado)").setDescription(`Compra feito com successo e com segurança.`);
        embed.addFields([
            {
                name: "`🆔` ID do pedido:",
                value: `\`\`\`#${orderProps.id}\`\`\``
        ...
        const viewOnWebsiteButton = new _discord.ButtonBuilder().setLabel(`Visualizar no site`).setStyle(_discord.ButtonStyle.Link).setURL(`${storeEntity.url}/checkout/${orderProps.id}`);
        await channel.send({
            embeds: [
                embed
            ],
...
SendPrivateSalesLogUsecase = _ts_decorate([
...
], SendPrivateSalesLogUsecase);

//# sourceMappingURL=send-private-sales-log.usecase.js.map