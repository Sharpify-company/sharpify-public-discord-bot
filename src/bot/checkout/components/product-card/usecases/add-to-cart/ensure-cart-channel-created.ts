...
import { ChannelType, InteractionResponse, PermissionsBitField, TextChannel } from "discord.js";
...
export async function EnsureCartChannelCreated({
...
}): Promise<Either<InteractionResponse, { channel: TextChannel; same: boolean }>> {
	...
	const channelParrent = await interaction.client.channels.cache.get(dotEnv.CHECKOUT_CATEGORY_ID);
	if (!channelParrent) {
		return failure(
			await interaction.reply({
	...
	if (user.cart.channelId) {
		...
		if (existingChannel) return success({ channel: existingChannel as TextChannel, same: true });
	}
